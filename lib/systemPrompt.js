export function buildSystemPrompt(learnedRules = {}) {
  ...
  // Tách rules có note và không có note
  const rulesWithNote = entries.filter(([, v]) => v.note)
  const rulesNoNote = entries.filter(([, v]) => !v.note)

  let learnedSection = ''
  if (entries.length > 0) {
    learnedSection = `\n\n## Các từ/cụm đã được xác nhận — ưu tiên áp dụng chính xác:`

    if (rulesNoNote.length > 0) {
      learnedSection += '\n' + rulesNoNote.map(([s, v]) => `- ${s} → ${v.bracket}`).join('\n')
    }

    if (rulesWithNote.length > 0) {
      learnedSection += '\n\n### Các rule có ghi chú đặc biệt (đọc kỹ trước khi áp dụng):\n'
      learnedSection += rulesWithNote.map(([s, v]) =>
        `- ${s} → ${v.bracket}\n  ⚠️ Ghi chú: ${v.note}`
      ).join('\n')
    }
  }
